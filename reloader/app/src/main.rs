...
use defmt_rtt as _;
use embassy_boot_nrf::{AlignedBuffer, BlockingFirmwareState as FirmwareState};
...
const BOOTLOADER_STATE: u32 = 0x0007F000;
...
async fn main(s: Spawner) {
    ...
    internal_flash.erase(BOOTLOADER_STATE, BOOTLOADER_STATE + 4096).unwrap();
    Timer::after(Duration::from_secs(1)).await;
    ...
    let mtx = Mutex::new(RefCell::new(internal_flash));
    ...
    let mut state = FirmwareState::new(part, &mut aligned.0);
